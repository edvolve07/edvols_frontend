...
function InstitutionFormModal({ institution, onClose, onSaved }) {
  const [form, setForm] = useState({
    ...
    basic_price: institution?.pricing?.basic_price ?? "",
    advanced_price: institution?.pricing?.advanced_price ?? "",
    professional_price: institution?.pricing?.professional_price ?? "",
      ...
      <form onSubmit={handleSubmit} className="relative w-full max-w-2xl rounded-2xl border border-slate-100 bg-white p-6 shadow-xl">
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          ...
          <div className="sm:col-span-2">
            ...
            <p className="mb-2 text-xs text-slate-500">Per-student negotiated price charged to this institution. Leave blank to use defaults (Level 1 Foundation ₹299 / Level 2 Skill Dev ₹599 / Level 3 Placement Ready ₹899).</p>
            <div className="grid gap-3 sm:grid-cols-3">
              <div>
                <label className="mb-1 block text-xs font-semibold text-slate-600">Level 1: Foundation (1–10)</label>
                <input className="field" type="number" min={0} placeholder="₹299" value={form.basic_price} onChange={(e) => update("basic_price", e.target.value)} />
              </div>
              <div>
                <label className="mb-1 block text-xs font-semibold text-slate-600">Level 2: Skill Dev (1–20)</label>
                <input className="field" type="number" min={0} placeholder="₹599" value={form.advanced_price} onChange={(e) => update("advanced_price", e.target.value)} />
              </div>
              <div>
                <label className="mb-1 block text-xs font-semibold text-slate-600">Level 3: Placement Ready (1–30)</label>
                <input className="field" type="number" min={0} placeholder="₹899" value={form.professional_price} onChange={(e) => update("professional_price", e.target.value)} />
              </div>
            </div>
          </div>
          <div className="sm:col-span-2">
