...
function AuraShader({ shape = 1.0, speed = 1.0, amplitude = 0.5, frequency = 0.5, scale = 0.2, blur = 1.0,
  color = DEFAULT_COLOR, colorShift = 1.0, brightness = 1.0,
  ...
  return (
    <div className={className} {...props}>
      <ReactShaderToy fs={shaderSource} devicePixelRatio={globalThis.devicePixelRatio ?? 1}
        uniforms={{
          uSpeed: { type: '1f', value: speed }, uBlur: { type: '1f', value: blur },
          uScale: { type: '1f', value: scale }, uShape: { type: '1f', value: shape },
          uFrequency: { type: '1f', value: frequency }, uAmplitude: { type: '1f', value: amplitude },
          uBloom: { type: '1f', value: 0.0 }, uMix: { type: '1f', value: brightness },
          uSpacing: { type: '1f', value: 0.5 }, uColorShift: { type: '1f', value: colorShift },
          uVariance: { type: '1f', value: 0.1 }, uSmoothing: { type: '1f', value: 1.0 },
...
export const AgentAudioVisualizerAuraVariants = cva(['aspect-square'], {
  variants: {
    size: { icon: 'h-[24px] gap-[2px]', sm: 'h-[56px] gap-[4px]', md: 'h-[112px] gap-[8px]',
      lg: 'h-[224px] gap-[16px]', xl: 'h-[448px] gap-[32px]' },
  },
  defaultVariants: { size: 'md' },
});

export const AgentAudioVisualizerAura = React.forwardRef(function AgentAudioVisualizerAura({
  size = 'lg', state = 'connecting', color = DEFAULT_COLOR, colorShift = 0.05, audioTrack, themeMode,
  className, ...props
}, ref) {
  const { speed, scale, amplitude, frequency, brightness } = useAgentAudioVisualizerAura(state, audioTrack);

  return (
    <AuraShader ref={ref} data-lk-state={state} blur={0.2} color={color} colorShift={colorShift}
      speed={speed} scale={scale} themeMode={themeMode} amplitude={amplitude} frequency={frequency}
      brightness={brightness}
      className={cn(AgentAudioVisualizerAuraVariants({ size }), className)} {...props} />
  );
});
