...
const uniformTypeToGLSLType = (t) => {
  switch (t) {
    ...
    default: console.error(`Invalid uniform type "${t}"`);
  }
};
...
function insertStringAtIndex(currentString, string, index) {
  return index > 0
...
export function ReactShaderToy({
  ...
  const processTextures = () => {
    ...
    if (textures && textures.length > 0) {
      ...
      const texturePromisesArr = textures.map((texture, id) => {
        ...
        texturesArrRef.current[id] = t;
        return t.load(texture).then((t) => {
          setupChannelRes(t, id);
        });
      });
      Promise.all(texturePromisesArr).then(() => onDoneLoadingTextures?.()).catch((e) => {
        onError?.(e);
        onDoneLoadingTextures?.();
      });
  ...
  const preProcessFragment = (fragment) => {
    ...
    if (!isValidPrecision) {
      onWarning?.(log(`wrong precision type ${precision}`));
    }
  ...
  const setUniforms = (timestamp) => {
    ...
    if (uniformsRef.current.iDate?.isNeeded) {
      ...
      gl.uniform4fv(dateUniform, [d.getFullYear(), d.getMonth() + 1, d.getDate(), d.getHours() * 60 * 60 + d.getMinutes() * 60 + d.getSeconds() + d.getMilliseconds() * 0.001]);
    }
  ...
  const drawScene = (timestamp) => {
    ...
    if (uniformsRef.current.iMouse?.isNeeded && lerp !== 1 && Array.isArray(mouseValue)) {
      mouseValue[0] = lerpVal(mouseValue[0] ?? 0, lastMouseArrRef.current[0] ?? 0, lerp);
      mouseValue[1] = lerpVal(mouseValue[1] ?? 0, lastMouseArrRef.current[1] ?? 0, lerp);
    }
...
class Texture {
  ...
  setupVideo = (url) => {
    ...
    const checkReady = () => { if (playing && timeupdate) this.isLoaded = true; };
  ...
  makePowerOf2 = (image) => {
    if (image instanceof HTMLImageElement || image instanceof HTMLCanvasElement || image instanceof ImageBitmap) {
      ...
      console.warn(log(`Image is not power of two ${image.width} x ${image.height}. Resized to ${this.pow2canvas.width} x ${this.pow2canvas.height}`));
  ...
  load = async (textureArgs) => {
    ...
    if (isImage === null && isVideo === null) return Promise.reject(new Error(log(`Please upload video or image (url: ${url})`)));
    ...
    if ((textureArgs.wrapS !== ClampToEdgeWrapping || textureArgs.wrapT !== ClampToEdgeWrapping ||
      (textureArgs.minFilter !== NearestFilter && textureArgs.minFilter !== LinearFilter)) && !isPowerOf2) {
