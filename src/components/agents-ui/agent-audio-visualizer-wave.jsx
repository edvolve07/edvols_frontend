...
function WaveShader({ speed = 10, color = '#1FD5F9', colorShift = 0.05, mix = 1.0, amplitude = 0.02,
  frequency = 20.0, lineWidth = 2.0, blur = 0.5, className, ...props }) {
  ...
  return (
    <div className={className} {...props}>
      <ReactShaderToy fs={shaderSource} devicePixelRatio={globalThis.devicePixelRatio ?? 1}
        uniforms={{
          uSpeed: { type: '1f', value: speed }, uAmplitude: { type: '1f', value: amplitude },
          uFrequency: { type: '1f', value: frequency }, uMix: { type: '1f', value: mix },
          uLineWidth: { type: '1f', value: lineWidth }, uSmoothing: { type: '1f', value: blur },
          uColor: { type: '3fv', value: rgbColor }, uColorShift: { type: '1f', value: colorShift },
        }}
...
export const AgentAudioVisualizerWaveVariants = cva(['aspect-square'], {
  variants: {
    size: { icon: 'h-[24px]', sm: 'h-[56px]', md: 'h-[112px]', lg: 'h-[224px]', xl: 'h-[448px]' },
  },
  defaultVariants: { size: 'lg' },
});

export const AgentAudioVisualizerWave = React.forwardRef(function AgentAudioVisualizerWave({
  size = 'lg', state = 'speaking', color, colorShift = 0.05, lineWidth, blur, audioTrack, className, ...props
}, ref) {
  ...
  return (
    <WaveShader ref={ref} data-lk-state={state} speed={speed} color={color} colorShift={colorShift}
      mix={opacity} amplitude={amplitude} frequency={frequency} lineWidth={_lineWidth} blur={blur}
      className={cn(AgentAudioVisualizerWaveVariants({ size }),
        'mask-[linear-gradient(90deg,transparent_0%,black_20%,black_80%,transparent_100%)]', className)} {...props}
    />
  );
});
