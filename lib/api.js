...
import { friendlyErrorMessage, networkError } from './errorMessages.js';
...
export async function apiFetch(path, options = {}) {
  ...
  if (!res.ok) {
    ...
    const message = friendlyErrorMessage(res.status, data, path);
    const error = new Error(message);
    error.status = res.status;
    error.data = { message, detail: message, details: [] };
    if (res.status === 423) error.locked = true;
    throw error;
  }

  if (typeof data === 'string') {
    throw new Error('We couldn’t load this information. Please refresh the page and try again.');
  }
  return data;
}
...
/* ─── Start interview ─── */
export async function startInterview(domain, role, resumeFile, useSaved = false, interviewNumber = null) {
  if (useSaved) {
    return apiFetch("/api/start", {
      ...
      body: JSON.stringify({
        ...
        role,
        use_saved: "true",
        ...(interviewNumber ? { interview_number: interviewNumber } : {}),
      }),
    });
  }
  ...
  if (interviewNumber) {
    form.append("interview_number", interviewNumber);
  }
  return apiFetch("/api/start", { method: "POST", body: form });
}

/* ─── Submit text answer ─── */
export async function submitTextAnswer(sessionId, answer, questionNumber = null) {
...
export async function submitAnswer(
  ...
  if (videoMetrics) {
    form.append("video_metrics", JSON.stringify(videoMetrics));
  }
  ...
  if (videoBlob) {
    ...
    return apiFetch('/api/answer_video_with_audio', {
      method: "POST",
      body: form,
    });
  } else {
    form.append("video", audioBlob, `audio.${audioExt}`);
    return apiFetch('/api/answer_video', {
      method: "POST",
      body: form,
    });
  }
...
async function downloadPdf(path, filename) {
  ...
  if (!res.ok) {
    ...
    try {
      ...
      if (text) {
        try { data = JSON.parse(text); } catch { /* Ignore non-JSON server error pages. */ }
      }
    } catch {}
    throw new Error(friendlyErrorMessage(res.status, data, path));
  }
  const blob = await res.blob();
  if (blob.size === 0) throw new Error('Your download isn’t ready yet. Please try again shortly.');
...
export function downloadCommunicationReportPdf(sessionId) {
  return downloadPdf(`/api/communication/student/reports/${sessionId}/pdf`, `coaching_report_${sessionId}.pdf`);
}
...
export function downloadAdminExport(type, format = "xlsx", filters = {}) {
  const params = new URLSearchParams({ format });
  if (filters.department_id) params.set("department_id", filters.department_id);
  if (filters.year) params.set("year", filters.year);
  if (filters.search) params.set("search", filters.search);
  return downloadPdf(`/api/admin/exports/${type}?${params.toString()}`, `${type}.${format}`);
}
...
export function downloadMentorshipOverviewPdf(departmentId) {
  const qs = departmentId ? `?department_id=${departmentId}` : "";
  return downloadPdf(`/api/mentorship/admin/reports/overview${qs}`, `branch_overview_report.pdf`);
}

export function downloadMentorshipStudentPdf(studentId) {
  return downloadPdf(`/api/mentorship/admin/reports/student/${studentId}`, `student_report_${studentId}.pdf`);
}

/* ─── Aptitude ─── */
export async function getAptitudeQuestions(domain, count = 20) {
  return apiFetch(`/api/aptitude/questions?domain=${encodeURIComponent(domain)}&count=${count}`);
}

export async function submitAptitude(domain, answers) {
  return apiFetch('/api/aptitude/submit', {
    method:  "POST",
...
export async function getSavedResume() {
  return apiFetch("/api/mentorship/resume/saved");
}

/* ─── Next interview (blueprint title + number) ─── */
export async function getNextInterview() {
  return apiFetch("/api/mentorship/interview/next");
}

export async function saveResume(file) {
  const form = new FormData();
  form.append("resume", file);
  return apiFetch("/api/mentorship/resume/saved", { method: "PUT", body: form });
}

export async function deleteSavedResume() {
  return apiFetch("/api/mentorship/resume/saved", { method: "DELETE" });
}

/* ─── Referral ─── */
export async function getMyReferral() {
  return apiFetch("/api/referral/my");
}

export async function getReferralHistory() {
  return apiFetch("/api/referral/history");
}

export async function validateReferralCode(code) {
  return apiFetch(`/api/referral/validate-public?code=${encodeURIComponent(code)}`);
}

/* ─── Master Admin Referral ─── */
export async function getReferralStats() {
  return apiFetch("/api/master/referral/stats");
}

export async function getReferralCampaigns(params = {}) {
  const query = new URLSearchParams(params).toString();
  return apiFetch(`/api/master/referral/campaigns${query ? `?${query}` : ''}`);
}

export async function getReferralCampaign(id) {
  return apiFetch(`/api/master/referral/campaigns/${id}`);
}

export async function createReferralCampaign(data) {
  return apiFetch("/api/master/referral/campaigns", {
    method: "POST",
    body: JSON.stringify(data),
  });
}

export async function updateReferralCampaign(id, data) {
  return apiFetch(`/api/master/referral/campaigns/${id}`, {
    method: "PUT",
    body: JSON.stringify(data),
  });
...
export async function exportReferralReport() {
  return apiFetch("/api/master/referral/export");
}

export async function getReferralWallet() {
  return apiFetch("/api/referral/wallet");
}

export async function requestReferralPayout(data) {
  return apiFetch("/api/referral/payout-request", {
    method: "POST",
    body: JSON.stringify(data),
  });
}
...
export async function getReferralProgramSettings() {
  return apiFetch("/api/master/referral/settings");
}
...
export async function getAdminPayoutRequests(params = {}) {
  const query = new URLSearchParams(params).toString();
  return apiFetch(`/api/master/referral/payouts${query ? `?${query}` : ''}`);
}

export async function processAdminPayout(id, data) {
  return apiFetch(`/api/master/referral/payouts/${id}`, {
    method: "PUT",
    body: JSON.stringify(data),
  });
}
